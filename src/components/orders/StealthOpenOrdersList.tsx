...
import { useGetMarketStealthOrders } from "@/api/orderbook/getMarketStealthOrders";
import type { StealthUserOrder } from "@/api/orderbook/getStealthOrders";
...
interface StealthOpenOrdersListProps {
  marketId: number;
}

export function StealthOpenOrdersList({ marketId }: StealthOpenOrdersListProps) {
  const { data: openOrders, isLoading } = useGetMarketStealthOrders({ marketId });
  ...
  return (
    <Card className="bg-surface-container-low border border-border shadow-none rounded-none flex flex-col overflow-hidden">
      <div className="px-6 md:px-8 py-6 flex justify-between items-center">
        <Heading className="text-lg font-bold tracking-tight text-white uppercase tracking-[0.05em] flex items-center gap-2">
          Open Orders
    ...
    <TableRow className="border-b border-border/40 hover:bg-surface-container transition-colors group">
      <TableCell className="px-6 md:px-8 py-4">
        <div className="flex flex-col xl:flex-row xl:items-center gap-2">
          ...
          <span className="w-fit text-[9px] uppercase tracking-widest font-sans font-bold text-violet-400/70 bg-transparent border border-violet-500/20 px-2 py-0.5 rounded-none">
            {order.orderType}
          </span>
        </div>
      </TableCell>
